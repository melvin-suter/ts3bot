...
import { getFirestore, Firestore, doc, getDoc, setDoc, Timestamp } from 'firebase/firestore/lite';
...
export class DB {
    ...
    async getUserData(clientID: string) : Promise<UserData> {
        let userDocument = <UserData>(await getDoc(doc(this.db,'users', clientID))).data();
        if(!userDocument) {
            userDocument = { 
                coin: 0, 
                lastDailyCoin: new Timestamp(0, 0)
            };
